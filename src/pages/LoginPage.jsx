...
export default function LoginPage() {
  ...
  const handleSignup = async (e) => {
    ...
    setLoading(true)
    setError(null)
    const { error: err, needsEmailConfirm } = await signUp({ email, password, username, displayName: displayName || username })
    setLoading(false)
    if (err) {
      setError(err.message)
      return
    }
    if (needsEmailConfirm) setConfirmEmailSent(true)
  }
        ...
        {confirmEmailSent ? (
          <div className="p-5 text-center space-y-3">
            ...
            <p className="text-sm text-gray-700 font-medium">
              <strong>{email}</strong>(으)로 인증 메일을 보냈어요.<br />메일함에서 인증 링크를 클릭한 뒤 로그인해주세요.
            </p>
            <button
              onClick={() => { setConfirmEmailSent(false); setTab('login') }}
