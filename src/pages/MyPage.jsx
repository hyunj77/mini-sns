...
import { Grid2X2, ChevronRight, Flame, Edit3, LogOut, Camera } from 'lucide-react'
import { useAuth } from '../context/AuthContext'
import BottomNav from '../components/BottomNav'
import PostCard from '../components/PostCard'
import { fetchPostsByUser, deletePost } from '../lib/posts'
...
export default function MyPage() {
  ...
  useEffect(() => {
    ...
    if (user) {
      fetchPostsByUser(user.id).then(setMyPosts)
  ...
  const handleDelete = async (postId) => {
    ...
    if (user) setMyPosts(await fetchPostsByUser(user.id))
  }
  ...
  const handleAvatarChange = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      img.onload = () => {
        ...
        updateProfile({ avatar_url: resized }).then(() => setUploading(false))
      }
        ...
        <div className="flex items-center gap-4">
          <div className="relative">
            <img
              src={avatar}
              alt="avatar"
              className={`w-20 h-20 rounded-full object-cover transition-opacity ${uploading ? 'opacity-50' : ''}`}
            ...
            <button
              ...
              className="absolute bottom-0 right-0 w-7 h-7 bg-[#00D4FF] rounded-full flex items-center justify-center border-2 border-white press shadow-md"
            >
              <Camera className="w-3.5 h-3.5 text-white" />
            </button>
            <input ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={handleAvatarChange} />
            {streak > 0 && (
              <div className="absolute -top-1 -left-1 bg-orange-500 text-white text-[10px] font-black px-1.5 py-0.5 rounded-full border-2 border-white">
                🔥{streak}
              </div>
            )}
          </div>
